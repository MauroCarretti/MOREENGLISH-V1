 ...
 * EduBlink Core
 *
 * Elementor widget for MasterStudy LMS Courses.
 *
 * @since 1.0.0
 */
class Courses extends \EduBlinkCore\Widgets\Courses {
    ...
    public function get_name() {
        return 'edublink-ms-courses';
    }

    public function get_title() {
        return __( 'Courses( Grid / Carousel / Filter )( MasterStudy LMS )', 'edublink-core' );
    }

    public function get_keywords() {
        return [ 'edublink', 'query', 'courses', 'lms', 'masterstudy', 'archive', 'loop', 'grid', 'slider', 'carousel', 'filter' ];
    }
    ...
    protected function render_query( $query, $settings, $single_wrapper ) {
        while ( $query->have_posts() ) : $query->the_post();
            ...
            ?>
            <div id="post-<?php the_ID(); ?>" <?php edublink_masterstudy_course_class( $single_wrapper ); ?> <?php echo esc_attr( $animation_attribute ); ?>>
    ...
    protected function render() {
        ...
        $this->add_render_attribute( 'container', 'class', 'eb-masterstudy-archive-courses' );
