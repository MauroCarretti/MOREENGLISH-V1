...
class Course_Filter extends Widget_Base {
    ...
    public function get_name() {
        return 'edublink-ss-course-filter';
    }
    ...
    public function get_title() {
        return __( 'Course Filter Sidebar(Sensei LMS)', 'edublink-core' );
    }
	...
	public function get_keywords() {
		return [ 'edublink', 'query', 'courses', 'lms', 'sensei lms', 'archive', 'loop','filter', 'sidebar filter', 'filter sidebar' ];
	}
    ...
    protected function _register_controls() {
        ...
        $this->add_control(
            'list_style',
            [
                'label'     => __( 'List Course Style', 'edublink-core' ),
                'type'      => Controls_Manager::SELECT,
                'default'   => '18',
    ...
    protected function render() {
        ...
        $params = Filter::query( $filtered_category, $filtered_tags, $settings );

        $paged = ( get_query_var( 'paged' ) ) ? get_query_var('paged') : 1;
        $params['posts_per_page'] = $settings['per_page']['size'] ? $settings['per_page']['size'] : -1;
        $params['paged'] = $paged;
            ...
            echo '<div ' . $this->get_render_attribute_string( 'wrapper' ) . '>';
                ...
                echo '<div class="edublink-col-lg-9 filter-course-column">';
                    echo '<div class="filtered-courses">';
                        ...
                        $args = apply_filters( 'edublink_ss_course_filter_args', $params );
                        ...
                        if ( $query->have_posts() ) :
                                ...
                                echo '<div ' . $this->get_render_attribute_string( 'grid' ) . '>';
                                    while ( $query->have_posts() ) : $query->the_post();
                                        ...
                                        ?>
                                            ...
                                            <?php
                                                include ( locate_template( 'sensei/custom/course-block/blocks.php', false, false, compact( 'args' ) ) );
                                            echo '</div>';  
                                        echo '</div>';  
                                    endwhile;
                                    wp_reset_postdata();
                            ...
                            if ( 'grid' !== $settings['content_type'] ) :
                                $this->add_render_attribute( 'list', 'class', 'display-layout-list edublink-course-archive' );
