 ...
 * EduBlink Core
 *
 * Elementor widget for Lifter LMS Courses.
 *
 * @since 1.0.0
 */
class Courses extends \EduBlinkCore\Widgets\Courses {
    ...
    public function get_name() {
        return 'edublink-ll-courses';
    }

    public function get_title() {
        return __( 'Courses( Grid / Carousel / Filter )( Lifter LMS )', 'edublink-core' );
    }

    public function get_keywords() {
        return [ 'edublink', 'query', 'courses', 'lms', 'lifter', 'archive', 'loop', 'grid', 'slider', 'carousel', 'filter' ];
    }
    ...
    protected function render_query( $query, $settings, $single_wrapper ) {
        while ( $query->have_posts() ) : $query->the_post();
            ...
            ?>
            <div id="post-<?php the_ID(); ?>" <?php edublink_lifter_course_class( $single_wrapper ); ?> <?php echo esc_attr( $animation_attribute ); ?>>
                <?php 
    ...
    protected function render() {
        ...
        $this->add_render_attribute( 'container', 'class', 'eb-lifter-archive-courses' );
