...
class Wishlist extends \EduBlinkCore\Widgets\Course_Wishlist {
    ...
    public function get_keywords() {
        return [ 'edublink', 'query', 'courses', 'wishlist', 'lms', 'lp', 'learnpress' ];
    }
    ...
    protected function render() {
        ...
        $pid = (array) get_user_meta( get_current_user_id(), '_lpr_wish_list', true );
        $args = array(
            'post_type'           => 'lp_course',
            'post__in'            => $pid,
            'post_status'         => 'publish',
            ...
            echo '<div class="edublink-row">';
                while ( $query->have_posts() ) : $query->the_post();
                    ...
                    <div id="post-<?php the_ID(); ?>" <?php post_class( $this->grid( $settings ) ); ?>  <?php echo esc_attr( $animation_attribute ); ?>>
                        <?php learn_press_get_template( 'custom/course-block/blocks.php', compact( 'block_data' ) );
                    echo '</div>';  
                endwhile;
            echo '</div>';  
        else :
