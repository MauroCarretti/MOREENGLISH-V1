...
class EduBlink_SS_Course_Review {
	...
	public function comments_template_locator( $template ) {
	    if ( self::POST_TYPE === get_post_type() ) :
			if ( is_edublink_ss_rating_enable() ) :
	...
	public static function update_average_ratings( $post_id ) {
	    ...
	    if ( self::POST_TYPE === $post_type ) :
	        ...
	        $courses = edublink_sensei_get_courses( $args );
	...
	public static function display_review( $rate, $style = 'default' ) {
		global $post;
		$rate = $rate ? $rate : 0;
		$rating = EduBlink_SS_Course_Review::get_all_reviews( get_the_ID() );
		$average_rating = EduBlink_SS_Course_Review::get_average_ratings( get_the_ID() );
...
EduBlink_SS_Course_Review::instance();